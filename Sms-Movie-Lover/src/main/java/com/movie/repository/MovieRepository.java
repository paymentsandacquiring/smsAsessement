	...
	public void getDisplayMovie() {
		try {
			...
			while(resultSet.next()) {
				...
				System.err.println(this.movieResults.getMovieGenre() + " Publisher: " + this.movieResults.getMoviePublisher());
				DisplayMovie displayMovie = new DisplayMovie(movieName, movieYear, this.movieResults.formatGenre(), this.movieResults.formatPublisher());
	...
	public void getMoviePublishers(String pId) {
		try {
			...
			while(resultSet.next()){
				...
				this.movieResults.setMoviePublisher(moviePublisher);
			}
		}catch (SQLException sql) {
			this.movieLogger.addEXCEPTION("Error fetching data " + sql.getMessage());
		}
